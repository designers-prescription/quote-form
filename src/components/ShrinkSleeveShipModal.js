...
const ShrinkSleevesShipModal = ({ quote, onClose }) => {
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
            <div className="flex items-center justify-center min-h-screen">
                ...
                <div className="relative bg-white rounded-lg p-6 m-2 w-7/12">
                    <div className="text-lg font-semibold mb-4">Edit Quote - Shrink Sleeves</div>
                        ...
                        {Object.keys(additionalData).map((field) => (
                                ...
                                <React.Fragment key={field}>
                                    {realTimeQuote.productShipping?.[field] ? (
                                        <p className='text-sm p-2 mb-4 rounded-md border border-dashed border-slate-500 bg-slate-50'> <span className='tracking-wide font-bold leading-6 text-gray-900'>{formatFieldName(field)}: </span> {realTimeQuote.productShipping[field]}</p>
