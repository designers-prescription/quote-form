...
const ShrinkSleevesShipModal = ({ quote, onClose, userRole }) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(
      doc(db, "QuoteRequirements", quote.id),
      (doc) => {
      ...
      <div className="flex items-center justify-center min-h-screen">
        ...
        <div className="relative bg-white rounded-lg p-6 m-2 w-7/12">
          <div className="text-lg font-semibold mb-4">
            Edit Shipping Pricing - Shrink Sleeves
          </div>
          <div className="text-sm font-semibold mb-4">
            Packaging Requirements - Shrink Sleeves
          </div>
          <div className="mb-4 grid text-sm grid-cols-4">
            {/* Render dynamic P tag based on realTimeQuote */}
            {Object.entries(realTimeQuote.product.fields)
              .sort((a, b) => a[0].localeCompare(b[0]))
              .map(([key, value]) => (
                <p
                  key={key}
                  className="p-2 m-1 rounded-md border border-dashed border-slate-500 bg-slate-50"
                >
                  <span className="tracking-wide font-bold leading-6 text-gray-900">
                    {formatFieldName(key)}:{" "}
                  </span>
                  {typeof value === "object" && value !== null
                    ? `${value.width} Width(in Inches) x${value.height} Height(in Inches)`
                    : value}
                </p>
              ))}
          </div>
          <div className="text-sm font-semibold mb-4">
            Packaging Pricing - Shrink Sleeves
          </div>
          <div className="mb-4 grid text-sm grid-cols-4">
            {Object.entries(realTimeQuote.productPricing)
              .sort((a, b) => a[0].localeCompare(b[0]))
              .map(([key, value]) => (
                <p
                  key={key}
                    ...
                    <>
                      {Object.entries(value).map(([key, val]) => (
                        <span className="block" key={key}>
                          <p className="block tracking-wide text-sm font-bold leading-6 text-gray-900">
                            {formatFieldName(key)}:
                          </p>
                          {val}
                        </span>
                      ...
                      {field === "shipBoxDimensions" ? (
                        <>
                          <div className="col-span-1">
                            <label
                              htmlFor="length"
                              className="tracking-wide text-sm font-bold leading-6 text-gray-900"
                            >
                              Length
                            </label>
                            <input
                              ...
                              onChange={(e) =>
                                handleChange(e, "shipBoxDimensions")
                              }
...
export default ShrinkSleevesShipModal;
